...
<head>
    <meta charset="UTF-8">
    <title><?php echo \Wasf\View\Blade::yield('title') ?: config('app.name'); ?> - <?php echo htmlspecialchars((string)(config('app.name') ?? ""), ENT_QUOTES, "UTF-8"); ?><</title>
    ...
    <!-- OpenGraph -->
    <meta property="og:title" content="<?php echo \Wasf\View\Blade::yield('title'); ?>">
    ...
    <meta property="og:site_name" content="<?php echo htmlspecialchars((string)(config('app.name') ?? ""), ENT_QUOTES, "UTF-8"); ?>">
    <meta property="og:type" content="website">
    <meta property="og:image" content="/assets/img/opengraph.png">

    <!-- Twitter Card -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="<?php echo \Wasf\View\Blade::yield('title'); ?>">
    ...
    <div class="navbar navbar-expand-lg landing-navbar px-3 px-md-4">
      ...
      <div class="navbar-brand app-brand d-flex align-items-center py-0 me-4">
        <a href="/" class="app-brand-link d-flex align-items-center text-decoration-none">
          <span class="fw-bold fs-4"><?php echo htmlspecialchars((string)(config('app.name') ?? ""), ENT_QUOTES, "UTF-8"); ?></span>
        </a>
      </div>
      ...
      <ul class="navbar-nav ms-auto flex-row gap-3 align-items-center">
        ...
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle p-0" data-bs-toggle="dropdown">
            <img src="<?php echo htmlspecialchars((string)(auth()->user()->photo ? '/uploads/profile/' . auth()->user()->photo : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) ?? ""), ENT_QUOTES, "UTF-8"); ?>"
                 class="avatar-sm shadow-sm">
          </a>
          <ul class="dropdown-menu dropdown-menu-end shadow">
            <li class="dropdown-item"><?php echo htmlspecialchars((string)(auth()->user()->name ?? ""), ENT_QUOTES, "UTF-8"); ?></li>
...
<main class="container py-5 fade-up mt-nav">
    <?php echo \Wasf\View\Blade::yield('content'); ?>
</main>

<!-- FOOTER -->
<footer class="text-center py-4 text-secondary small">
    <?php echo htmlspecialchars((string)(config('app.name') ?? ""), ENT_QUOTES, "UTF-8"); ?> v<?php echo htmlspecialchars((string)(config('app.version') ?? ""), ENT_QUOTES, "UTF-8"); ?> — © <?php echo htmlspecialchars((string)(date('Y') ?? ""), ENT_QUOTES, "UTF-8"); ?>
</footer>
